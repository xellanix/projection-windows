...
function ExtractProcess({ setIsBuild }: { setIsBuild: (v: null | boolean) => void }) {
    ...
    useEffect(() => {
        const task = async () => {
            ...
            const res = await window.extract();
    ...
    return (
        <div className="flex flex-col h-full gap-8 overflow-hidden">
            <div className="flex flex-col gap-4 h-full overflow-hidden justify-center items-center">
                ...
                <h1 className="text-center font-bold text-2xl">Importing...</h1>
                <p className="text-center text-muted-foreground">
                    Please wait a few seconds while we import your projection file. <br />
        ...
        <AnimatePresence mode="wait">
            <motion.div
                ...
                {isBuild === false ? (
                    ...
                    <ExtractProcess setIsBuild={setIsBuild} />
                )}
            </motion.div>
        </AnimatePresence>
    );
